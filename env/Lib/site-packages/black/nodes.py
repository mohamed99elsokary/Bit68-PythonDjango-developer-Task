...
class Visitor(Generic[T]):
    ...
    def visit(self, node: LN) -> Iterator[T]:
        ...
        if visitf := getattr(self, f"visit_{name}", None):
...
def first_leaf_column(node: Node) -> Optional[int]:
    ...
    return next(
        (child.column for child in node.children if isinstance(child, Leaf)),
        None,
    )
...
def unwrap_singleton_parenthesis(node: LN) -> Optional[LN]:
    ...
    return None if lpar.type != token.LPAR or rpar.type != token.RPAR else wrapped
